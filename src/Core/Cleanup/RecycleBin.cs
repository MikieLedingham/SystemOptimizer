...
namespace SystemOptimizer.Core.Cleanup
{
    ...
    public static class RecycleBin
    {
        ...
        public sealed class RestoreResult
        {
            ...
            /// <summary>
            /// Already at its original path before this restore reached it - because a
            /// parent folder brought it back, or because a previous restore did.
            ///
            /// Counted apart from Problems deliberately. The item IS back, which is what
            /// the user asked for, and calling that "could not be moved back" reports a
            /// success as a failure.
            /// </summary>
            public int AlreadyBack { get; set; }
        ...
        public static RestoreResult Restore(IEnumerable<string> originalPaths)
        {
            ...
            // SHALLOWEST FIRST, and skip anything a restored folder already brought back.
            //
            // A cleanup plan can list a folder AND items inside it - crash-dump folders are
            // the everyday case, each holding one file. Restoring those as an unordered set
            // fails for one of them whichever way round it goes, and a real run lost exactly
            // its 43 folders to this:
            //
            //   file first   - restoring the file calls Directory.CreateDirectory on its
            //                  parent, which RECREATES the very folder that was recycled.
            //                  The folder's own turn then finds its path occupied.
            //   folder first - the folder comes back with its contents, so every file
            //                  inside it then finds ITS path occupied.
            //
            // Both were reported as "could not be moved back", which was not true: the data
            // was back either way. Restoring parents first makes the nested entries
            // genuinely redundant, and they are then counted as restored, because they are.
            var ordered = wanted
                .OrderBy(p => p.Count(c => c == System.IO.Path.DirectorySeparatorChar))
                .ThenBy(p => p, StringComparer.OrdinalIgnoreCase)
                .ToList();

            var restoredFolders = new List<string>();

            foreach (var original in ordered)
            {
                // Did a folder restored a moment ago already bring this back?
                bool broughtBack = false;
                foreach (var folder in restoredFolders)
                {
                    if (original.StartsWith(folder, StringComparison.OrdinalIgnoreCase))
                    {
                        broughtBack = true;
                        break;
                    }
                ...
                // Already sitting at its original path, whatever the bin still thinks.
                //
                // Checked BEFORE the bin lookup, because both routes end in the same place:
                // restoring on top of an existing file is refused anyway, so attempting it
                // only converts a success into an exception. Running Restore twice used to
                // report hundreds of items as "could not be moved back" when every one of
                // them was exactly where the user wanted it.
                ...
                try
                {
                    bool isFolder = Directory.Exists(binPath);
                    RestoreOne(binPath, original);
                    result.Restored++;
                    if (isFolder)
                    {
                        restoredFolders.Add(original.TrimEnd(System.IO.Path.DirectorySeparatorChar)
